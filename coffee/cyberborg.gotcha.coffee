...
class Gotcha
  ...
  start: (event) ->
    ...
    for coordinate, danger_level of position
      continue unless typeof danger_level is 'number' and /^\d+,\d+$/.test(coordinate)
      if danger_level > @ai.too_dangerous
        Trace.green "Danger area: #{coordinate} #{danger_level.to_i()}"
      else
