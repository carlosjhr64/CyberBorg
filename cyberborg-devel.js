...
var Ai, BASE, CORDER_PASS, Command, CyberBorg, DERRICKS, DORDER_MAINTAIN, FACTORIES, FORDER_MANUFACTURE, Group, IS_LAIDOFF, LABS, LORDER_RESEARCH, SCOUTS, Scouter, Trace, WZArray, WZObject, ai, allowed_hqless_build, base_group, bug_report, cyberBorg, eventChat, eventDestroyed, eventDroidBuilt, eventDroidIdle, eventResearched, eventStartLevel, eventStructureBuilt, gotcha_idle, gotcha_rogue, gotcha_selected, gotcha_working, gotchas, script, start_trace, trace_command,
  __slice = [].slice;
...
Group = (function() {
  ...
  Group.prototype.execute = function(command) {
    ...
    if (count === 0 && (command.savings != null)) {
      cyberBorg.power -= command.savings;
    }
    return count;
  };
...
start_trace = function(event) {
  ...
  if (droid = event.droid) {
    return ai.trace.out("\tDroid: " + (droid.namexy()) + "\tID:" + droid.id + "\tCost: " + droid.cost);
  }
};

trace_command = function(command) {
  var at, key, keyvals;
  keyvals = [];
  for (key in command) {
    switch (key) {
  ...
  return ai.trace.blue(keyvals.sort().join(' '));
};
  ...
  if (command = droid.command) {
    ...
    if (order === 0) {
      ...
      if (order !== droid.dorder) {
        ai.trace.out("\t\tBUG: Order changed.");
      }
    }
  }
  if (event.name === 'Destroyed') {
    return ai.trace.out("\t\t" + ((_ref = event.group) != null ? _ref.name : void 0) + "'s " + (event.object.namexy()) + " was destroyed.");
  }
};
...
gotcha_working = function(droid, command) {
  ...
  if (droid.executes(command)) {
    order = command.order;
    if (ai.trace.on) {
      return ai.trace.green("\tRe-issued " + ("" + (order.order_map()) + ", #" + order + ", to " + droid.name + "."));
    }
  } else {
    return ai.trace.red("\t" + droid.name + " is a lazy bum!");
  }
};
  ...
  Ai.prototype.events = function(event) {
    ...
    switch (event.name) {
      case 'StartLevel':
        ...
        break;
      case 'StructureBuilt':
        this.structureBuilt(event.structure, event.droid, event.group);
        break;
      case 'DroidBuilt':
        this.droidBuilt(event.droid, event.structure, event.group);
        break;
      case 'DroidIdle':
        this.droidIdle(event.droid, event.group);
        break;
      case 'Researched':
        this.researched(event.research, event.structure, event.group);
        break;
      case 'Destroyed':
        this.destroyed(event.object, event.group);
        break;
      case 'Chat':
        this.chat(event.sender, event.to, event.message);
        break;
      default:
        this.trace.red("" + event.name + " NOT HANDLED!");
    }
