...
function is_idle(droid){
  if (droid.type == STRUCTURE) {
    // It's not really a droid  :P
    return(structureIdle(droid));
  }
  // It's a droid
  var not_idle = [
    DORDER_BUILD,
...
function Group(group,orders,reserve){
  ...
  this.buildDroid=function(order){
    var factories = this.group.factories().idle();
    for(var i=0;i<factories.length;i++){
      if(buildDroid(factories[i],
        order.name, order.body, order.propulsion, "", order.droid_type, order.turret)){
        return(factories[i]);
      }
    }
    return(null);
  ...
  this.build=function(order){
    var builders = []; // going to return the number of builders
    var structure = order.structure;
    if (isStructureAvailable(structure)) {
      var at = order.at;  // where to build the structure
      // Get available trucks
      var trucks = this.group.trucks().idle();
      ...
      if (trucks.length > 0) {
        ...
        var pos = pickStructLocation(trucks[0], structure, at.x, at.y);
        if (pos) {
