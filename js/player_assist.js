...
function eventStartLevel(){
  ...
  */
  console(
    "We have " + reserve.group.length + " droids available, and " +
  ...
  */
  var derricks = cyberBorg.get_resources(reserve.group.center());
  ...
  */
  ...
  /*
    ...
    will then be relayed down to an individual droids.
...
function base_group()
{
  var base = cyberBorg.base;
  var order = base.orders.next();
  if (order) {
    var builders = base.build(order);
    ...
    if (builders.length==0) { console("Base group was unable to complete base orders."); }
  } else {
...
debug('Factory group length '+cyberBorg.factory.group.length);
    factory_group();
  }
  ...
  if (structure.stattype == HQ) { min_map_and_design_on(structure); }
...
function min_map_and_design_on(structure){
...
function factory_group(){
  // FACTORY_ORDERS is a list of droids to build, and
  // we build them one at a time.
  var factory = cyberBorg.factory;
  var order = factory.orders.next();
  if (order) {
...
function eventDroidBuilt(droid,structure){
  ...
  if (cyberBorg.factory.group.contains(structure)) { factory_group(); }
}
