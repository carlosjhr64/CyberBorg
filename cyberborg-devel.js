...
var Ai, BASE, CORDER_PASS, Command, CyberBorg, DERRICKS, DORDER_MAINTAIN, FACTORIES, FORDER_MANUFACTURE, Group, IS_LAIDOFF, LABS, LORDER_RESEARCH, SCOUTS, Scouter, Trace, WZArray, WZObject, ai, allowed_hqless_build, base_group, bug_report, cyberBorg, eventChat, eventDestroyed, eventDroidBuilt, eventDroidIdle, eventResearched, eventStartLevel, eventStructureBuilt, gotcha_idle, gotcha_rogue, gotcha_selected, gotcha_working, gotchas, script, start_trace, trace, trace_command,
  __slice = [].slice;
...
start_trace = function(event) {
  ...
  if (droid = event.droid) {
    return trace.out("\tDroid: " + (droid.namexy()) + "\tID:" + droid.id + "\tCost: " + droid.cost);
  }
};
...
bug_report = function(label, droid, event) {
  ...
  if (event.name === 'Destroyed') {
    return trace.out("\t\t" + ((_ref = event.group) != null ? _ref.name : void 0) + "'s " + (event.object.namexy()) + " was destroyed.");
  }
};
  ...
  Ai.prototype.events = function(event) {
    ...
    switch (event.name) {
      ...
      case 'Chat':
        ...
        break;
      default:
        trace.red("" + event.name + " NOT HANDLED!");
    }
